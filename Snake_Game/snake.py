...
class Collision:
    ...
    def betwenn_head_and_body(self, snake):
...
def main():
    ...
    pygame.display.set_caption('SNAKE by Ammar_Taha73')
    ...
    while True:
        ...
        if collision.between_snake_and_apple(snake, apple):
            ...
            snake.add_body()
            score.increase()
        if snake.state != 'STOP' or snake.state != 'QUIT':
            snake.move_body()
            snake.move_head()
        ...
        if collision.betwenn_head_and_body(snake):
            #lose
            snake.die()
            apple.spawn()
            score.reset()
